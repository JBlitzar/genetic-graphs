        ...
        match mutationType:
            ...
            case "skip":
                ...
                while True:
                    ...
                    if (
                        dag.has_one_predecessor(candidate.name)
                        and hasattr(candidate, "shape_transform")
                        and candidate.realShape != None
                    ):
                        break
                ...
                while True:
                    ...
                    if (
                        i > 1000
                        or dag.has_one_predecessor(candidate_2.name)
                        and hasattr(candidate_2, "shape_transform")
                        and candidate_2.get_output_shape() == candidate.realShape
                        and candidate_2 != candidate
                        and len(list(dag.graph.predecessors(candidate_2.name))) >= 1
                    ):
                        break
                    i += 1
                ...
                try:
                    ...
                    if (connector.input_connections < 2) or len(
                        list(dag.graph.predecessors(connector.name))
                    ) < 2:  # hack
                        raise ValueError
            ...
            case "smallbig":
                u, v = dag.randomEdge()
                dag.insertChainBetween(
                    [rand_of_transform((1, 0.5, 0.5)), rand_of_transform((1, 2, 2))],
                    u,
                    v,
                )
            case "inout":
                ...
                print(u, v)
                dag.insertChainBetween(
                    [rand_of_transform((2, 1, 1)), rand_of_transform((0.5, 1, 1))], u, v
                )
            case "downup":
                u, v = dag.randomEdge()
                dag.insertChainBetween(
                    [rand_of_transform((2, 0.5, 0.5)), rand_of_transform((0.5, 2, 2))],
                    u,
                    v,
                )
            case "cull":
                ...
                while i < 1000:
                    ...
                    if (
                        dag.has_one_predecessor(candidate.name)
                        and hasattr(candidate, "shape_transform")
                        and (all(x == 1 for x in candidate.shape_transform))
                        and candidate.num_outputs == 1
                    ):
                        break
            ...
            case "unsmallbig":
                ...
                for u, v in dag.graph.edges:
                    ...
                    if dag.graph.out_degree(u) == 1 and dag.graph.in_degree(v) == 1:
                        ...
                        if (
                            hasattr(middle_node, "shape_transform")
                            and dag.graph.out_degree(middle_node) == 1
                            and dag.graph.in_degree(middle_node) == 1
                        ):

                            next_node = list(dag.graph.successors(middle_node))[0]
                            if (
                                hasattr(next_node, "shape_transform")
                                and dag.graph.out_degree(next_node) == 1
                                and dag.graph.in_degree(next_node) == 1
                            ):

                                if middle_node.shape_transform == (
                                    1,
                                    0.5,
                                    0.5,
                                ) and next_node.shape_transform == (1, 2, 2):

                                    dag.remove_connection(u, middle_node.name)
                                    dag.remove_connection(
                                        middle_node.name, next_node.name
                                    )
                        ...
                        if (
                            ...
                            if (
                                ...
                                if middle_node.shape_transform == (
                                    ...
                                    1,
                                    1,
                                ) and next_node.shape_transform == (0.5, 1, 1):
                        ...
                        if (
                            ...
                            if (
                                ...
                                if middle_node.shape_transform == (
                                    ...
                                    0.5,
                                    0.5,
                                ) and next_node.shape_transform == (0.5, 2, 2):
