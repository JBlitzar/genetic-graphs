...
class Module(nn.Module):
    ...
    def validate_inout(self):
        return (
            self.input_connections == self.num_inputs
            and self.output_connections == self.num_outputs
        )
    ...
    def forward(self, inputs):
        ...
        if len(inputs) != self.num_inputs:
            raise ValueError(
                f"{self.name} expects {self.num_inputs} inputs, but got {len(inputs)}."
            )
...
class ModuleDag(Module):
    ...
    def add_module(self, module):
        def get_child_attributes(child_class):
            ...
            child_specific_attrs = [
                a for a in child_specific_attrs if not a.startswith("_")
            ]
    ...
    def add_connection(self, from_module, to_module):
        if (
            from_module not in self.modules_moduledag
            or to_module not in self.modules_moduledag
        ):
            raise ValueError(
                f"Both modules must exist in the graph before connecting them. {from_module in self.modules_moduledag} {to_module in self.modules_moduledag}"
            )
        ...
        if not nx.is_directed_acyclic_graph(self.graph):
            ...
            raise ValueError(
                "Adding this connection creates a cycle, which is not allowed."
            )
        ...
        for module_name, module in self.modules_moduledag.items():
            ...
            # Validate all other modules normally
                ...
                if not module.validate_inout():
                    ...
                    # print(f"{module.}")
                    raise ValueError(
                        f"{module_name} does not have the correct number of input/output connections. {module.input_connections} {module.output_connections}"
                    )
